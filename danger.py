...
def redeem_key(message):
    ...
    # Remove the key from the list after redemption
...
def approve_list_command(message):
    try:
        ...
        if not approved_users:
            ...
            response = "\n".join([f"User ID: {user['user_id']}, Valid Until: {user['valid_until']}" for user in approved_users])
            bot.send_message(message.chat.id, response, parse_mode='Markdown')
    except Exception as e:
...
def send_welcome(message):
    ...
    # Welcome message
    welcome_message = (f"Welcome to Powerful DDOS, @{username}!\n\n"
